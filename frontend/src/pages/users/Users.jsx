...
const emptyForm = { fullName: '', email: '', password: '', role: 'satis' };
    ...
    <div>
      ...
      {showForm && (
        <form
          ...
          className="mb-6 grid grid-cols-1 gap-4 rounded-xl bg-white/70 p-5 shadow-sm sm:grid-cols-2 lg:grid-cols-4"
        >
          <Input
            id="fullName"
            label="Ad Soyad"
      ...
      {isLoading ? (
        ...
        <div className="overflow-x-auto rounded-xl bg-white/70 shadow-sm">
          <table className="w-full text-left text-sm">
            ...
            <tbody>
              {users.map((u) => (
                <tr key={u.id} className="border-b border-brand-primary/5 last:border-0">
                  ...
                  <td className="px-4 py-3">
                    <select
                      ...
                      className="rounded-lg border border-brand-primary/30 px-2 py-1 text-xs focus:outline-none focus:ring-1 focus:ring-brand-primary disabled:opacity-50"
                    >
                      {ROLES.map((role) => (
                        <option key={role} value={role}>
                          {ROLE_LABELS[role]}
